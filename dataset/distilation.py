...
class Distilation(Dataset):
    def __init__(self, base_url,split,dataset_cfg,**kwargs):
        ...
        self.resize = Resize(self.data_cfg['vid_transform']['IMAGE_SIZE'])
    ...
    def read_one_view(self,name,selected_index,width,height):
        ...
        center_clip_no_crop_hand = []
       
        path = 'data/VN_SIGN/'+f'videos/{name}'   
        ...
        for frame,frame_index in zip(frames,selected_index):
            ...
            if 'center' in name:
                if not self.is_train:
                    new_frame = self.resize(frame)
                    center_clip_no_crop_hand.append(self.transform(new_frame,cnt = range(2,5))) # cnt >= 2 will omit Scale and Center Crop
                else:
                    center_clip_no_crop_hand.append(self.transform(frame)) 
        ...
        if 'center' in name:
            center_clip_no_crop_hand = torch.stack(center_clip_no_crop_hand,dim = 0)
            return clip,poseflow,center_clip_no_crop_hand
    ...
    def read_videos(self,center,left,right):
        ...
        if max_vlen - min_vlen < 10:
            ...
            center_video,center_pf,center_clip_no_crop_hand = self.read_one_view(center,selected_index,width=c_width,height=c_height)
        ...
        return center_video,center_pf,left_video,left_pf,right_video,right_pf,center_clip_no_crop_hand
        # return 1
    ...
    def __getitem__(self, idx):
        ...
        center_video,center_pf,left_video,left_pf,right_video,right_pf,center_clip_no_crop_hand = self.read_videos(center,left,right)
        
        return center_video,center_pf,left_video,left_pf,right_video,right_pf,center_clip_no_crop_hand,torch.tensor(label)
